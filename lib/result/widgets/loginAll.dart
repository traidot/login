...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<void> _loginWithGoogle(BuildContext context) async {
    try {
      ...
      final user = await fetchData(_currentUser!.email);

      print('Result:  ${user['result']}!');
      print('Message:  ${user['message']}.');
      print('Access Token:  ${user['access_token']}.');
      print('User ID:  ${user['user_id']}.');
      if (user['result'].toString() == "100") {
        if (!context.mounted) return;
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) =>
                  ResultPage(user['message'], user['access_token'])),
        );
      } else {
        if (!context.mounted) return;
        Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => ResultPage(user['message'], '')),
        );
      }
    } catch (error) {
      print(error);
  ...
  Future<void> _loginWithLine(BuildContext context) async {
    try {
      ...
      final user = await fetchData(userEmail!);
    ...
    try {
      ...
      switch (res.status) {
        case FacebookLoginStatus.success:
          ...
          final user = await fetchData(email!);
  ...
  Future<Map<String, dynamic>> fetchData(String email) async {
    final response = await http
        .get(Uri.parse('http://127.0.0.1:8000/api/auth/email?email=$email'));
